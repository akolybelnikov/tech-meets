...
const App: React.FC = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            const cities: AxiosResponse = await axios('https://tech-meets-api.herokuapp.com/cities')
            setCities(cities.data)

            const events: AxiosResponse = await axios('https://tech-meets-api.herokuapp.com/events')
            setEvents(events.data.sort(
                (a: TechEvent, b: TechEvent) => (a.startDate > b.startDate) ? 1 : -1
            ))

            const user: AxiosResponse = await axios('https://tech-meets-api.herokuapp.com/user')
    ...
    const leaveEvent = async (id: number) => {
        try {
            await axios.delete(`https://tech-meets-api.herokuapp.com/user/${id}`)
            fetchUserEvents()
        } catch (e) {
            console.error(e)
        }
